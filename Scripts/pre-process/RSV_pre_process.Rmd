...
- Following imputation and a second round of QC associate infant genotype with the probablity of RSV infection and symptom severity (Resvinet score)
- Investigate 'top hits' at a genome-wide level, in addition to investigating candidate genes of interest
...
Very conviently different arrays use different annotations\
Therefore need to update SNP names in R/Plink for later comparisons
Gathered annotation file from https://www.thermofisher.com/uk/en/home/life-science/microarray-analysis/microarray-data-analysis/genechip-array-annotation-files.html
...
#When we convert to VCF this = chr:bp:SNP anyways so we can convert to rsID after that I think
```

#### Liftover:

Data is in GChr37 build. Use UCSC Liftover to update to 38 build.
...
```
Update Bim file with liftover SNPs
...
Generate TOPMED38 vcf.gz file to use and tabulate using bcftools
